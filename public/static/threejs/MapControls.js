...
THREE.MapControls = function ( object, domElement ) {
	...
	// 发生鼠标事件的时候会调用该函数
	// 执行update会执行ookAt，可能会与你代码中的lookAt冲突
	this.update = function () {
		...
		return function update() {
			...
			// .target属性是MapControls对象的内部相机目标观察点
			// 执行lookAt，通过内部相机目标观察点和改变参数相机的姿态,注意这里的lookAt和其它位置的lookAt代码冲突
			// 与其它位置lookAt冲突原因：关键在于参数scope.target
			scope.object.lookAt( scope.target );
			// console.log('scope',scope);//控制台打印scope指向THREE.MapControls对象
			//默认值0,0,0 初始值不受MapControls的相机参数影响
			// 鼠标左键拖动平移，才会改变scope.target，鼠标中键和右键不影响scope.target，也就是缩放和旋转不影响
			// console.log('scope.target',scope.target);
			// 查看鼠标事件对相机位置的影响
			// 左键平移影响位置 右键旋转影响位置 缩放影响透视投影相机位置
